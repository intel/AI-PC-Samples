...
def dynamic_preprocess(image, min_num=2, max_num=6, image_size=768, use_thumbnail=False):
    ...
    if len(processed_images) != blocks:
        raise RuntimeError("Image preprocessing produced an unexpected number of blocks.")
...
def convert_deepseek_ocr2(model_id=model_id, model_path=None, quantization_config=None, revision=None):
    from transformers import AutoModel, AutoTokenizer

    if model_path is None:
        model_path = Path(model_id.split("/")[-1])
    model_path = Path(model_path)
    tokenizer = AutoTokenizer.from_pretrained(model_id, local_files_only=True, revision=revision)
    ...
    def infer(
        ...
        if "<image>" in conversation[0]["content"] and save_results:
            ...
            if "line_type" in outputs:
                ...
                geometry = ast.literal_eval(outputs)
                lines = geometry["Line"]["line"]
                line_type = geometry["Line"]["line_type"]
                # print(lines)

                endpoints = geometry["Line"]["line_endpoint"]
