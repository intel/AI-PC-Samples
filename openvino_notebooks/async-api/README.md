...
This example includes its helper module and test video locally. An internet connection is required only on the first run to download the Open Model Zoo model.

We recommend running the notebook in a virtual environment:

```text
uv sync
uv run jupyter lab async-api.ipynb
```
