...
This notebook downloads the referenced Hugging Face tokenizer and models on its first run, then reuses the local copies on subsequent runs. The compressed TinyLlama model download is approximately 1.1 GB.

We recommend running the notebook in a virtual environment:

```text
uv sync
uv run jupyter lab openvino-tokenizers.ipynb
```
